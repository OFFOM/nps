...
// ajax正确返回
func (s *BaseController) AjaxOkWithId(str string, id int, vkey string) {
	s.Data["json"] = ajaxWithId(str, 1, id, vkey)
	s.ServeJSON()
	s.StopRun()
}
...
// 组装ajax
func ajaxWithId(str string, status int, id int, vkey string) map[string]interface{} {
	json := make(map[string]interface{})
	json["status"] = status
	json["msg"] = str
	json["id"] = id
